...
router.post('/rooms', (req, res) => {
  try {
    const { name = 'Landscape Game', totalRounds = 2, canvasWidth = 1200, canvasHeight = 800, scenarioId } = req.body || {};
    const session = createRoom(name, totalRounds, canvasWidth, canvasHeight, scenarioId || undefined);
    res.json({
      session: {
        id: session.id,
