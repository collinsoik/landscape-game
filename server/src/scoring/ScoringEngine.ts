...
import { getRulesForPair, getAllPatterns, CROSS_ZONE_BONUS, DIVERSITY_BONUS, DIMINISHING_RETURNS } from './InteractionMatrix';
...
export function computeScore(placements: Placement[], zones: ZoneRect[]): ScoreBreakdown {
  ...
  // 1. Base scores from each placed element (with diminishing returns for duplicates)
  const typeCounts = new Map<string, number>();
  for (const p of placements) {
    const def = getElement(p.elementType);
    if (!def) continue;

    const count = typeCounts.get(p.elementType) || 0;
    const multiplier = getDiminishingMultiplier(count);
    typeCounts.set(p.elementType, count + 1);
  ...
  // 5. Diversity bonus: nonlinear scaling rewards using many different categories
  const categoriesUsed = new Set(
    placements.map((p) => getElement(p.elementType)?.category).filter(Boolean)
  );
  const diversityBonus = DIVERSITY_BONUS.thresholds[
    Math.min(categoriesUsed.size, DIVERSITY_BONUS.thresholds.length - 1)
  ];
