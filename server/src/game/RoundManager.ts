...
function mapRound(row: any): Round {
  return {
    ...
    availableCategories: row.available_categories ? JSON.parse(row.available_categories) : null,
  };
}
...
export function startRound(
  ...
  onTickHook?: (remaining: number) => void
): Round {
  ...
  state.interval = setInterval(() => {
    ...
    if (onTickHook) onTickHook(state.remaining);
